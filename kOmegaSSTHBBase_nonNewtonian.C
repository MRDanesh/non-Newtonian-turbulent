...
void kOmegaSST<TurbulenceModel, BasicTurbulenceModel>::correct()
{
...

    {
        ...
        (
         ...
         ==
          ...
          + Qsas(S2(), gamma, beta)
          // Added E_nn = (alpha/mu_t)*(chiNN + xiNN)
          + (alpha_/max(muEff(), dimensionedScalar("small", muEff().dimensions(), SMALL))) * (chiNN + xiNN)
          + omegaSource()
    ...
    // --- Non-Newtonian closure variables ---
    volScalarField gammaDot = sqrt(2*S2() + dimensionedScalar("small", dimless, SMALL));

    // Apparent viscosity with Papanastasiou regularization
    volScalarField muApp =
        (tau0_*(1 - exp(-m_*gammaDot)) + K_*pow(gammaDot, n_)) / gammaDot;

    // Derivative of viscosity
    volScalarField dmu_dgamma =
        ((n_ - 1)*K_*pow(gammaDot, n_)
        - tau0_*(1 - exp(-m_*gammaDot))
        + m_*gammaDot*tau0_*exp(-m_*gammaDot)) / (gammaDot*gammaDot);

    // Non-Newtonian eddy viscosity
    volScalarField muNN = dmu_dgamma*rho_*betaStar_*k_*omega_ / (muApp*gammaDot);

    // Non-Newtonian production and diffusion
    volScalarField chiNN = -muNN*S2();
    volScalarField xiNN = fvc::laplacian((dmu_dgamma*S2()/gammaDot), k_);
...
tmp<fvScalarMatrix> kEqn
    (
     ...
     ==
      ...
      // Added chiNN and xiNN as non-Newtonian source terms
+ chiNN
+ xiNN
+ kSource()
